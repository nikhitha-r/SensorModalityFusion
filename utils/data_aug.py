...
# List of the available augmentation techniques
...
aug_dir = "/storage/remote/atcremers61/w0017/Noisy_data1/images_aug/"

with open("/usr/prakt/w0017/try_avod_moe/avod_moe/avod/split/val.txt", "r") as valFile:
    num_set = sum(1 for line in valFile)
    # Consider 40% of the val set to be augmented
    aug_num = int(0.4 * num_set)
    
with open("/usr/prakt/w0017/try_avod_moe/avod_moe/avod/split/val.txt", "r") as valFile:
    with open("/usr/prakt/w0017/try_avod_moe/avod_moe/avod/split/aug.txt", "w") as augFile:
        num_list = []
        for i in valFile:
            num_list.append(i)
        image_id = "7480"

        # Loop through the number of aug samples to be created
        for i in range(0, aug_num):
            ...
            augFile.write("\n")
            # Make a random choice of the sug technique
            random_aug = random.choice(aug_list)
            orig_image = cv2.imread(image_dir + random_line.strip() + ".png", 1)
            # Apply the augmentation on the image
            image_mod = eval(random_aug)(orig_image)

            image_id = int(image_id) + 1
            # Save the augmented images at a certain location
            cv2.imwrite(aug_dir + "00" + str(image_id) + ".png", image_mod)

velo_dir = "/storage/remote/atcremers61/w0017/KITTI/PointRCNN/KITTI/object/training/velodyne/"
calib_dir = "/storage/remote/atcremers61/w0017/KITTI/PointRCNN/KITTI/object/training/calib/"
label_dir = "/storage/remote/atcremers61/w0017/KITTI/PointRCNN/KITTI/object/training/label_2/"

aug_lidar_dir = "/storage/remote/atcremers61/w0017/Noisy_data1/velodyne_noise/"
aug_calib_dir = "/storage/remote/atcremers61/w0017/Noisy_data1/calib_noise/"
aug_label_dir = "/storage/remote/atcremers61/w0017/Noisy_data1/label_noise/"

planes_dir = "/storage/remote/atcremers61/w0017/KITTI/planes/"
aug_planes_dir = "/storage/remote/atcremers61/w0017/Noisy_data1/planes_noise/"

new_id = 7480
with open("/usr/prakt/w0017/try_avod_moe/avod_moe/avod/split/aug.txt", "r") as augFile:
    for id in augFile:
        new_id = int(new_id) + 1
        # Lidar
        src_lidar = velo_dir + str(id).strip() + ".bin"        
        dest_lidar = aug_lidar_dir + "00" + str(new_id) + ".bin"
        shutil.copy(src_lidar, dest_lidar)
        
        # Calib
        src_calib = calib_dir + str(id).strip() + ".txt"
        dest_calib = aug_calib_dir + "00" + str(new_id) + ".txt"
        shutil.copy(src_calib, dest_calib)

        # Label
        src_label = label_dir + str(id).strip() + ".txt"
        dest_label = aug_label_dir + "00" + str(new_id) + ".txt"
        shutil.copy(src_label, dest_label)

        # Planes
        src_planes = planes_dir + str(id).strip() + ".txt"        
        dest_planes = aug_planes_dir + "00" + str(new_id) + ".txt"
        shutil.copy(src_planes, dest_planes)
