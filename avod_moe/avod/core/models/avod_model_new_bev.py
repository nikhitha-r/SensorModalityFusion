...
import avod.core.cont_fusion_generator as cf
from avod.builders import feature_extractor_builder
from wavedata.tools.core import calib_utils


class AvodModelBEV(model.DetectionModel):
    ##############################
    ...
    def __init__(self, model_config, train_val_test, dataset):
        ...
        super(AvodModelBEV, self).__init__(model_config)
    ...
    def scale_bev(self, bev_rois, img_rois):
        val_to_mul = tf.divide(self.max_img_feature_val, self.max_bev_feature_val)
        return tf.multiply(bev_rois, val_to_mul)

    def scale_img(self, bev_rois, img_rois):
        val_to_mul = tf.divide(self.max_bev_feature_val, self.max_img_feature_val)
        return tf.multiply(bev_rois, val_to_mul)
...
# 
        ...
        _, bev_mar_boxes_norm = cf.add_margin_to_regions(bev_proposal_boxes, 
                                                        self.dataset.kitti_utils.bev_extents)
                                                        

        bev_pixels_loc = cf.bev_pixel_eq_1_loc(self._rpn_model._bev_preprocessed)
        ...
        height_list = [min_height+(2*x+1)*(max_height-min_height)/(2.0*num_slices) for x in range(num_slices)]
        print("bev_preprocess shape: ", (self._rpn_model._bev_preprocessed).shape)

        velo_pc = cf.bev_pixel_loc_to_3d_velo(bev_pixels_loc, tf.shape(self._rpn_model._bev_preprocessed)[1:3], 
                                    height_list, self.dataset.kitti_utils.bev_extents)
        print("PL_CALIB_P2 shape: ", self._rpn_model.placeholders[RpnModel.PL_CALIB_P2].shape)

        p_2d = anchor_projector.project_to_image_tensor(tf.transpose(tf.cast(velo_pc,tf.float32)), 
                                self._rpn_model.placeholders[RpnModel.PL_CALIB_P2])

        print("image feature maps [0] shape: ", img_feature_maps[0].shape)
        features_at_p_2d = tf.gather_nd(img_feature_maps[0], tf.cast(tf.round(tf.transpose(p_2d)), tf.int32))

        print("features_at_p_2d shape: ", features_at_p_2d.shape)
        new_bev = cf.create_fused_bev(tf.shape(self._rpn_model._bev_preprocessed), 
                                     bev_pixels_loc, features_at_p_2d)
        ...
        ####################################################################################
        # TODO PROJECT: use new bev for fusion
        fc_output_layers = \
            avod_fc_layers_builder.build(
                layers_config=avod_layers_config,
                input_rois=[bev_rois, new_bev_rois],
                input_weights=[bev_mask, img_mask],
                num_final_classes=self._num_final_classes,
                box_rep=self._box_rep,
                top_anchors=top_anchors,
