...
class Evaluator:
    ...
    def __init__(self,
        ...
        ##########################################################################################
        # TODO PROJECT: include new bev model
        self.full_model = isinstance(self.model, AvodModel)\
    ...
    def run_checkpoint_once(self, checkpoint_to_restore):
        ...
        ##########################################################################################
        # TODO PROJECT: create weights dir
        ...
        while current_epoch == self.model.dataset.epochs_completed:
            ...
            if validation:
                ...
                ##########################################################################################
                # TODO PROJECT: create weights and rois dir
                # weights_rois_dir = predictions_base_dir + '/weights/{}/{}'.format(data_split, global_step)
                # trainer_utils.create_dir(weights_rois_dir)
            ...
            #if num_valid_samples % 100 == 1 or img_weights_larger.eval(feed_dict, self._sess):
                ...
                # print("img_weight: ", img_weight)
                # print("bev_weight: ", bev_weight)
                # bev_pre = self.model._rpn_model._bev_preprocessed.eval(feed_dict, self._sess)
                # print("bev_pre max: ", np.max(bev_pre), "bev_pre min: ", np.min(bev_pre))
                # print(bev_pre)
                ...
                # print("img_feat_whole max: ", np.max(img_feat_whole))
                # print("img_feat: ", img_feat)
                # print("bev_feat: ", bev_feat)
                # print("img_feat max: ", np.max(img_feat))
                # print("bev_feat max: ", np.max(bev_feat))
                ...
                #    print("Weights and regions for {}th sample successfully saved!".format(num_valid_samples))
    ...
    def run_latest_checkpoints(self):
        ...
        if ckpt_indices is not None:
            ...
            for ckpt_idx in ckpt_indices:
                ...
                ##############################################################################
                # TODO PROJECT: if we start training from checkpoints 100000, ckpt_idx = ckpt_idx - 100
                # ckpt_idx -= 100
                ##############################################################################
    ...
    def get_evaluated_ckpts(self,
        ...
        if model_name == 'avod_model':
            avg_loss_file_path = predictions_base_dir + '/avod_avg_losses.csv'
        #######################################################################
        # TODO PROJECT: support avod_bev_new
        elif model_name == 'avod_model_new_bev':
            avg_loss_file_path = predictions_base_dir + '/avod_new_bev_avg_losses.csv'
        #######################################################################
        else:
            avg_loss_file_path = predictions_base_dir + '/rpn_avg_losses.csv'
