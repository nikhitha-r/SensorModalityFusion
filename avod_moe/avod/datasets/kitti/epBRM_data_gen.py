...
class epBRMDataset():
    ...
    def get_samples(self, batch_size=1):
        ...
        samples = self.kitti_dataset.next_batch(
        ...
        data_aug_scale_extents = [
            self._config["data_aug_scale_min"], self._config["data_aug_scale_max"]]
        data_aug_rot_extents = [
            self._config["data_aug_rot_min"], self._config["data_aug_rot_max"]]
        ...
        for ind in range(center_gt.shape[0]):
            ...
            sample_points_center_gt = sample_points - \
                center_gt[ind].reshape((1, 3))

            # rotate sample points -angle_gt[ind] to make the bounding box aligned with y axis
            rot_sample_points_center_gt = self.rotate_z(
                sample_points_center_gt, -angle_gt[ind])
            ...
            random_angle = np.random.uniform(
                data_aug_rot_extents[0], data_aug_rot_extents[1])
            new_angle_gt[ind] = angle_gt[ind] + random_angle
            rotate_back_sample_points = self.rotate_z(
                scaled_rot_sample_points_center_gt, new_angle_gt[ind])
    ...
    def cylinder_filter(self, point_cloud, center, radius, zmin, zmax, sample_num):
        ...
        print("FILTERED", filtered_point_cloud)
    ...
    def random_translate(self, point_cloud, center_gt, dist_bound):
        ...
        assert tran.shape[1] == point_cloud.shape[1]
